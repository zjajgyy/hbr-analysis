...
<form action="upload" method="post" enctype="multipart/form-data" target="upload_iframe" onsubmit="showStatus(); ">
...
//            $('statusInfo').innerHTML = '已完成百分比: ' + ss[0] + '% <br />已完成数(M): ' + ss[1] + '<br/>文件总长度(M): ' + ss[2] + '<br/>传输速率(K): ' + ss[3] + '<br/>已用时间(s): ' + ss[4] + '<br/>估计总时间(s): ' + ss[5] + '<br/>估计剩余时间(s): ' + ss[6] + '<br/>正在上传第几个文件: ' + ss[7];
            ...
            var w = (req.responseText.bytesRead/req.responseText.contentLength) + "%";
            console.log(w);
            $('progressBarItem').style.width = w;
